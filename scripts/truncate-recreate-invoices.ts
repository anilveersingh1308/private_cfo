...
async function truncateAndRecreateInvoices() {
  try {
    ...
    insertedInvoices.forEach((invoice, index) => {
      console.log(`   ${index + 1}. ${invoice.invoice_number} - ${invoice.client_name} - $${invoice.total_amount} (${invoice.status})`);
    });

    console.log('\n🎉 Invoice table truncation and recreation completed successfully!');
